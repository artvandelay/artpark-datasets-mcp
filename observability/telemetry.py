"""
Telemetry middleware for ARTPARK MCP Server.
Adapted from esankhyiki-mcp (nso-india/esankhyiki-mcp).
    ...
    async def on_call_tool(self, context: MiddlewareContext, call_next):
        ...
        with self._tracer.start_as_current_span(f"tool.{tool_name}") as span:
            ...
            if output_data is not None:
                ...
                # Log summary to stderr (full output goes to span attributes / Jaeger)
                print(f"[TELEMETRY] tool.{tool_name} -> {output_size} bytes", file=sys.stderr)
